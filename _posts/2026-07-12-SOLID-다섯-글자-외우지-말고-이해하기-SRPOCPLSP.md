...
그런데 솔직히 말하면, 다섯 개를 줄줄 외워서 대답은 해도 "그래서 내 코드에 어떻게 쓰는 건데?"까지 이어지는 경우는 드물더라고요. 저도 한동안 그랬습니다.
...
```javascript
class Employee {
  calculatePay() { ... }    // 회계팀이 바꿔달라고 함
  reportHours() { ... }     // 인사팀이 바꿔달라고 함
  save() { ... }            // 개발팀(DBA)이 바꿔달라고 함
}
```
...
class Square extends Rectangle {
  setWidth(w) { this.width = w; this.height = w; }  // 정사각형이니까
...
function test(rect) {
  rect.setWidth(5);
  rect.setHeight(4);
  console.log(rect.width * rect.height); // 20을 기대하지만 Square면 16
}
```
