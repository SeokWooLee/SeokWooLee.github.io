...
저도 처음 iOS 앱을 만들 때 네트워크 매니저, 유저 세션, 캐시 관리 전부 싱글톤으로 도배했었어요.

편하거든요. 어디서든 `Manager.shared`만 부르면 되니까요.

그런데 프로젝트가 커지면서 이상한 일이 생기기 시작했습니다. 테스트가 안 돼요. 화면 하나 고쳤을 뿐인데 엉뚱한 데서 버그가 터지고요.

그때 처음 "아, 싱글톤이 안티패턴 소리를 듣는 데는 이유가 있구나" 하고 느꼈습니다.

이 글에서는 Swift 싱글톤 패턴이 정확히 무엇인지, 왜 남용하면 안티패턴 취급을 받는지, 그럼에도 언제는 써도 되는지를 실제 경험 위주로 정리해볼게요.
